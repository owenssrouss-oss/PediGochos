...
class KitchenController {
  ...
  async openCentralSedeModal() {
    try {
      const res = await fetch('/api/settings');
      if (res.ok) {
        const settings = await res.json();
        if (document.getElementById('central-sede-name')) document.getElementById('central-sede-name').value = settings.central_delivery_name || '';
        if (document.getElementById('central-sede-lat')) document.getElementById('central-sede-lat').value = settings.central_delivery_lat !== null ? settings.central_delivery_lat : '';
        if (document.getElementById('central-sede-lng')) document.getElementById('central-sede-lng').value = settings.central_delivery_lng !== null ? settings.central_delivery_lng : '';
      }
    } catch (e) {
      console.warn('Error fetching settings:', e);
    }
    document.getElementById('central-sede-modal').classList.add('active');
  }

  closeCentralSedeModal() {
    document.getElementById('central-sede-modal').classList.remove('active');
  }

  getCurrentGPSForCentralSede() {
    if (navigator.geolocation) {
      this.showLocalToast('📡 Obteniendo GPS para Sede Central...');
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          const latEl = document.getElementById('central-sede-lat');
          const lngEl = document.getElementById('central-sede-lng');
          if (latEl) latEl.value = pos.coords.latitude;
          if (lngEl) lngEl.value = pos.coords.longitude;
          this.showLocalToast('✅ Coordenadas de Sede Central capturadas');
        },
        (err) => {
          alert('No se pudo obtener la ubicación GPS: ' + err.message);
        },
  ...
  async handleSaveCentralSede(e) {
    e.preventDefault();
    const name = document.getElementById('central-sede-name').value.trim();
    const lat = document.getElementById('central-sede-lat').value;
    const lng = document.getElementById('central-sede-lng').value;
    ...
    try {
      ...
      if (res.ok) {
        this.showLocalToast('✅ Sede Central de Domicilios guardada con éxito.');
        this.closeCentralSedeModal();
      } else {
        alert('Error al guardar la Sede Central.');
      }
    } catch (err) {
      console.error(err);
      alert('Error de conexión al guardar Sede Central.');
    }
  }
